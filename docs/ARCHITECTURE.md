...
## PWA Architecture Clarification

### Production Implementation

The ICUPA platform uses a **single Vite + React SPA** located in the `src/` directory as the primary production application. This app contains all three PWA surfaces:

1. **Client/Diner PWA** - Accessible at `/` (root route)
   - Implemented in: `src/components/client/`, `src/modules/client/`
   
2. **Merchant/Vendor PWA** - Accessible at `/merchant`
   - Implemented in: `src/components/merchant/`, `src/modules/merchant/`
   
3. **Admin Panel** - Accessible at `/admin`
   - Implemented in: `src/components/admin/`, `src/modules/admin/`
   - **Includes AI agent management** (`AgentSettingsPanel.tsx`)

**CI/CD Build:** The `pnpm build` command compiles this Vite app, and the output is deployed to production.

### AI Agent Management

All AI agents (AI Waiter, Allergen Guardian, Upsell Agent) are configured and managed through the **Admin Panel** at `/admin`:

- Agent autonomy levels (0-3)
- Budget limits (session and daily USD caps)
- Tool allow-lists and custom instructions
- Experiment flags and feature toggles
- Audit logs for all configuration changes

The admin panel connects to the `agents-service/` via Supabase Edge Functions, ensuring real-time synchronization of agent configurations across all running instances.

### Experimental Next.js Apps

The `apps/` directory contains **experimental** Next.js implementations that are NOT currently deployed to production:

- `apps/client/` - Standalone Next.js client PWA (experimental)
- `apps/vendor/` - Standalone Next.js merchant PWA (experimental)
- `apps/admin/` - Standalone Next.js admin panel (experimental)
- `apps/web/` - Unified Next.js app importing from `src/` (experimental)

These apps represent potential future migration paths to Next.js for improved SSR, SEO, and independent deployment capabilities. They are maintained for exploration but should not be confused with the production implementation.
...
**Future (Separate Next.js Apps):**
- ✅ Independent deployments per surface
- ✅ Better SEO for public pages
- ✅ Server-side rendering capabilities
- ✅ Smaller bundles per surface
