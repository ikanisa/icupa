...
type PaymentFailureIdentifier = string | { providerRef?: string | null; paymentId?: string | null };

export async function markPaymentFailed(
  client: SupabaseClient,
  identifier: PaymentFailureIdentifier,
  reason?: string
): Promise<void> {
  const failureReason =
    reason?.slice(0, 400) ?? "The payment provider reported that this payment did not complete.";
  ...
  if (typeof identifier === "string") {
    failureUpdate = await updateBuilder.eq("provider_ref", identifier);
  } else if (identifier.paymentId) {
    failureUpdate = await updateBuilder.eq("id", identifier.paymentId);
  } else if (identifier.providerRef) {
    failureUpdate = await updateBuilder.eq("provider_ref", identifier.providerRef);
  } else {
    console.error("markPaymentFailed called without a valid identifier");
    return;
  }
