...
-- Verify that only staff can requeue fiscalisation jobs and that the helper resets metadata.
...
begin
  begin
    perform public.retry_fiscalization_job(
      '00000000-0000-4000-9000-000000000801',
      '00000000-0000-4000-9000-000000000901'
    );
    raise exception 'Diners should not be able to retry fiscalisation jobs';
  exception
    when others then
      if position('permission_denied' in coalesce(sqlerrm, '')) = 0 then
        raise;
      end if;
  end;
end;
...
declare
  ...
  queued_status text;
begin
  select public.retry_fiscalization_job(
    '00000000-0000-4000-9000-000000000801',
    '00000000-0000-4000-9000-000000000901'
  )
  into retry_payload;

  if coalesce(retry_payload->>'msg_id', '') = '' then
    raise exception 'Retry helper should return a pgmq msg_id';
  end if;
  ...
  if queued_status is distinct from 'queued' then
    raise exception 'Retry should leave the fiscalisation job queued but found %', queued_status;
  end if;
end;
$$;

-- Queue summary helper should surface the queued job with timing metadata.
do $$
declare
...
begin
  select count(*)
    into summary_count
    from public.fiscalization_queue_summary();

  if summary_count <> 1 then
    raise exception 'Expected exactly one queue summary row but found %', summary_count;
  end if;

  select job_count, longest_wait_seconds, last_error
    into queued_count, queued_wait, summary_last_error
    from public.fiscalization_queue_summary()
   where status = 'queued';

  if queued_count <> 1 then
    raise exception 'Queued summary should report one job but found %', queued_count;
  end if;

  if queued_wait < 0 then
    raise exception 'Queued summary wait time should be non-negative';
  end if;

  if summary_last_error is not null then
    raise exception 'Queued summary should not surface an error message';
  end if;
end;
$$;
...
-- Clean up any remaining queue messages generated by the retry helper.
do $$
declare
  cleanup_msg record;
begin
  for cleanup_msg in select * from public.dequeue_fiscalization_job(300) loop
    perform public.delete_fiscalization_job(cleanup_msg.msg_id);
  end loop;
end;
$$;
