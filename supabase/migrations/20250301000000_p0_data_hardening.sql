...
create policy if not exists "Inventory visible to staff" on public.inventory_items
  for select using (
    is_staff_for_tenant(tenant_id, array['owner','manager','cashier','chef','admin','support']::role_t[])
  );

create policy if not exists "Inventory managed by staff" on public.inventory_items
  for all using (
    is_staff_for_tenant(tenant_id, array['owner','manager','cashier','chef','admin','support']::role_t[])
  )
  with check (
...
create policy if not exists "Item ingredients follow inventory" on public.item_ingredients
  for all using (
    exists (
...
create policy if not exists "Service role manages inventory" on public.inventory_items
  for all using (auth.role() = 'service_role')
  with check (auth.role() = 'service_role');

create policy if not exists "Service role manages ingredients" on public.item_ingredients
  for all using (auth.role() = 'service_role')
  with check (auth.role() = 'service_role');
...
create policy if not exists "Agent events visible to staff" on public.agent_events
  for select using (
...
create policy if not exists "Agent events managed by service role" on public.agent_events
  for all using (auth.role() = 'service_role')
  with check (auth.role() = 'service_role');
...
create policy if not exists "Service role manages webhook events" on public.payment_webhook_events
  for all using (auth.role() = 'service_role')
  with check (auth.role() = 'service_role');

create policy if not exists "Staff may view webhook audit" on public.payment_webhook_events
  for select using (
