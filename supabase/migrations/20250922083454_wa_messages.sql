...
begin
  if not exists (
    ...
    where schemaname = 'agents'
      ...
      and policyname = 'p_agents_messages_service'
  ) then
...
grant execute on function public.agent_message_exists(text) to service_role;
...
grant execute on function public.agent_latest_session(text) to service_role;
...
grant execute on function public.agent_recent_messages(int) to service_role;
