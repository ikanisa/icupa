...
const rawCardProvider = (import.meta.env.VITE_CARD_PAYMENTS_PROVIDER ?? "stripe").toLowerCase();
const CARD_PAYMENT_PROVIDER: "stripe" | "adyen" = rawCardProvider === "adyen" ? "adyen" : "stripe";

const CARD_PAYMENT_FUNCTION =
  CARD_PAYMENT_PROVIDER === "adyen"
    ? "payments/adyen/checkout"
    ...
    try {
      ...
      if (selectedMethod === "card") {
        ...
        const { data, error } = await supabase.functions.invoke<PaymentFunctionResponse>(
          CARD_PAYMENT_FUNCTION,
          {
            body: {
              ...basePayload,
              success_url: successUrl,
              cancel_url: cancelUrl,
              payment_provider: CARD_PAYMENT_PROVIDER,
            },
          }
        );
        if (error) {
          const providerLabel = CARD_PAYMENT_PROVIDER === "adyen" ? "Adyen" : "Stripe";
          throw new Error(error.message ?? `${providerLabel} checkout failed`);
        }
        response = data ?? null;
