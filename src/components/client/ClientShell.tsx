...
export function ClientShell() {
  ...
  const pushButtonDisabled =
    ...
    pushSubscription.isSubscribing ||
    pushSubscription.isUnsubscribing;
  ...
  const handleEnablePush = useCallback(async () => {
    ...
    if (success && typeof Notification !== "undefined" && Notification.permission === "granted") {
      toast({
        ...
        description: "We will alert you when your table status changes.",
      });
    }
  }, [pushSubscription]);
  ...
  const handleSendTestPush = useCallback(async () => {
    ...
    try {
      await pushSubscription.sendTestNotification();
      toast({
        title: "Test notification sent",
        description: "Check your notification tray for the ICUPA preview alert.",
      });
    } catch (_error) {
      toast({
        title: "We couldn’t show a test alert",
        description: "Ensure notifications are allowed for ICUPA and try again.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleDisablePush = useCallback(async () => {
    ...
    if (success) {
      toast({
        ...
        description: "You can re-enable alerts from this card at any time.",
      });
    } else {
      toast({
        title: "We couldn’t disable notifications",
        description: "Please try again or adjust your browser settings.",
        variant: "destructive",
      });
    }
  }, [pushSubscription]);

  const pushCardTitle = pushSubscription.isSubscribed ? "Notifications enabled" : "Enable instant updates";
  const pushCardDescription = pushSubscription.isSubscribed
    ? "We’ll alert you about receipts and table updates even if you close the app."
    : "Get notified when receipts are issued or an order status changes, even if you close the app.";
  const canSendTestAlert = pushSubscription.isSubscribed && pushSubscription.permission === "granted";
        ...
        <main id="main-content" className="flex-1 flex flex-col focus:outline-none" tabIndex={-1} role="main">
          ...
          {shouldShowPushCard && (
            <motion.div
              ...
              className="mx-4 mt-3"
            >
              <div className="glass-card rounded-2xl border border-border/40 p-4 space-y-3" role="status">
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-2 flex-1">
                    <div>
                      <p className="text-sm font-semibold">{pushCardTitle}</p>
                      <p className="text-xs text-muted-foreground">{pushCardDescription}</p>
                    </div>
                  ...
                  {!pushSubscription.isSubscribed ? (
                    ...
                    <>
                      <Button
                        ...
                        disabled={!canSendTestAlert || isSendingTestPush}
                        variant="secondary"
                        size="sm"
                        className="rounded-full"
                      >
                        {isSendingTestPush ? "Sending…" : "Send test alert"}
                      </Button>
                      <Button
                        onClick={handleDisablePush}
                        disabled={isDisablingPush || pushSubscription.isUnsubscribing}
                        variant="ghost"
                        size="sm"
                        className="rounded-full"
                      >
