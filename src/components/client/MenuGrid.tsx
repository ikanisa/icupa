...
import { Plus, Clock, Star, Leaf, Flame, Wine, Sparkles } from "lucide-react";
          ...
          <motion.div key={menuItem.id} variants={item}>
            ...
            >
              ...
              <CardContent className="p-4 space-y-3">
                {matchScores && typeof matchScores[menuItem.id] === "number" && (
                  <Badge
                    variant="secondary"
                    className="gap-1 text-[11px] font-medium uppercase tracking-wide bg-primary/15 text-primary border-primary/20 w-fit"
                  >
                    <Sparkles className="w-3 h-3" />
                    {Math.round(matchScores[menuItem.id] * 100)}% match
                  </Badge>
                )}
