...
import { PLATFORM_TENANT_ID, useAdminTenants } from "@/hooks/useAdminTenants";
...
export function AdminShell() {
  ...
  const isPlatformScope = activeTenant?.scope === "platform";
  const tenantIdForAgents = isPlatformScope ? PLATFORM_TENANT_ID : activeTenant?.id ?? null;
  const tenantIdForOperations = isPlatformScope ? null : activeTenant?.id ?? null;
  const currency = activeTenant?.currency === "EUR" ? "EUR" : "RWF";

  useEffect(() => {
    if (isPlatformScope && (activeTab === "analytics" || activeTab === "compliance")) {
      setActiveTab("agents");
    }
  }, [isPlatformScope, activeTab]);
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-6">
                  ...
                  <SelectContent>
                    {tenants?.map((tenant) => (
                      <SelectItem key={tenant.id} value={tenant.id} className="text-black">
                        {tenant.scope === "platform" ? `${tenant.name} (all tenants)` : tenant.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="glass-card grid h-12 grid-cols-4 gap-2 border border-white/10 bg-white/10 p-1 text-xs uppercase tracking-wide text-white/80">
            {TABS.map((tab) => (
              <TabsTrigger
                key={tab.id}
                value={tab.id}
                disabled={isPlatformScope && (tab.id === "analytics" || tab.id === "compliance")}
                className={cn(
          ...
          <TabsContent value="agents" className="mt-0">
            <AgentSettingsPanel
              tenantId={tenantIdForAgents}
              tenantName={activeTenant?.name ?? undefined}
          ...
          <TabsContent value="analytics" className="mt-0">
            <AnalyticsOverview
              tenantId={tenantIdForOperations}
              currency={currency}
              scope={isPlatformScope ? "platform" : "tenant"}
            />
          </TabsContent>
          ...
          <TabsContent value="compliance" className="mt-0">
            <CompliancePanel
              tenantId={tenantIdForOperations}
              tenantRegion={activeTenant?.region ?? "RW"}
              scope={isPlatformScope ? "platform" : "tenant"}
            />
          </TabsContent>
        </Tabs>
