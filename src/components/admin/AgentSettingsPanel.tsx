...
function AgentConfigCard({ tenantId, config, onSave, isSaving }: AgentConfigCardProps) {
  ...
  const isPlatformView = tenantId === PLATFORM_TENANT_ID;
  const showInheritedBanner = !isPlatformView && config.isInherited;
  ...
  function handleSubmit() {
    onSave({
      ...
      originTenantId: config.originTenantId,
      patch: {
    ...
    <Card className="glass-card flex flex-col gap-4 border border-white/10 bg-white/10 p-5 text-white">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {isPlatformView ? (
            ...
            <Badge className="bg-white/20 text-primary">Tenant override</Badge>
          )}
          {config.syncPending ? (
            <Badge variant="outline" className="border-amber-300 text-amber-200">Sync pending</Badge>
          ) : (
            <Badge className="bg-emerald-500/20 text-emerald-100">Synced</Badge>
          )}
      ...
      {showInheritedBanner ? (
        <p className="rounded-xl border border-white/10 bg-white/5 p-3 text-xs text-white/70">
          This agent currently uses platform defaults. Saving changes will create a tenant-specific override and log an audit
          trail.
        </p>
      ) : null}
...
export function AgentSettingsPanel({ tenantId, tenantName, scope }: AgentSettingsPanelProps) {
  const tenantKey = scope === "platform" ? PLATFORM_TENANT_ID : tenantId;
  const { data, isLoading, isError } = useAgentConfigs(tenantKey);
  const updateMutation = useUpdateAgentConfig(tenantKey);
    ...
    if (scope === "platform") {
      return (
        <div className="rounded-2xl border border-white/20 bg-white/10 p-4 text-sm text-white/70">
          Platform defaults are synced to every tenant when onboarding completes. Adjust instructions, budgets, and tools here to set the baseline for new venues.
        </div>
      );
    }
    ...
    <div className="grid gap-5">
      {scope === "platform" ? (
        <div className="rounded-2xl border border-white/20 bg-white/10 p-4 text-sm text-white/80">
          Editing <span className="font-semibold">platform defaults</span> updates the instructions and guardrails every tenant inherits across ICUPA. The agents service will still respect tenant-specific overrides where configured.
        </div>
      ) : tenantName ? (
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-white/70">
          Managing agent policies for <span className="font-semibold">{tenantName}</span>. Updates trigger a sync in the agents service and record an audit trail automatically.
        </div>
      ) : null}
      {data.map((config) => (
        <AgentConfigCard
          key={config.id}
          tenantId={tenantKey ?? PLATFORM_TENANT_ID}
          config={config}
