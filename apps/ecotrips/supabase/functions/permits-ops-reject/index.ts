...
function audit(fields: Record<string, unknown>) {
  console.log(
    JSON.stringify({
      level: "AUDIT",
      event: "permits.reject",
      fn: "permits-ops-reject",
      ...fields,
    }),
  );
}
...
async function rejectRequest(requestId: string, note: string | null) {
  if (!SUPABASE_URL || !SERVICE_ROLE_KEY) {
    throw new Error("Supabase configuration missing");
  }

  const response = await fetch(
    `${SUPABASE_URL}/rest/v1/rpc/reject_permit_request`,
    {
      method: "POST",
      headers: {
        apikey: SERVICE_ROLE_KEY,
  ...
  if (!response.ok) {
    const text = await response.text();
    const error = new Error(`Failed to reject permits.requests: ${text}`);
...
const handler = withObs(async (req) => {
  ...
  if (req.method === "GET" && url.pathname.endsWith("/health")) {
    return healthResponse("permits-ops-reject");
  }
  ...
  try {
    ...
    const rejectionNote = noteInput
      ? `${timestamp} - ${actor}: rejected (${noteInput})`
      : `${timestamp} - ${actor}: rejected`;

    await rejectRequest(requestIdInput, rejectionNote);
...
}, { fn: "permits-ops-reject", defaultErrorCode: ERROR_CODES.UNKNOWN });
