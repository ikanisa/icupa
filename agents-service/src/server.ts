...
interface RunnerStateJson {
  modelResponses?: RunnerUsage[];
}

interface RunnerResult {
  state?: {
    toJSON?: () => RunnerStateJson;
  };
}
...
function extractUsage(result: RunnerResult): UsageAccumulator {
  const stateJson = result?.state?.toJSON?.();
  if (!stateJson?.modelResponses) {
    return { inputTokens: 0, outputTokens: 0 };
  }

  return stateJson.modelResponses.reduce<UsageAccumulator>(
    (acc, response) => {
